...
var RaphaelPaper = function(raphaelAreaId,xmlDoc,workflowProcess){
	...
	$("#tools_"+tempId+" > div[type='PATH']").unbind("click").bind("click",function(){
		$(paper).data("mod","path");
	});
	$("#tools_"+tempId+" > div[type='POINTER']").unbind("click").bind("click",function(){
		$(paper).data("mod","pointer");
	});
/*	$("#tools_"+tempId+" > div[type='SHOW']").unbind("click").bind("click",function(){
		formatShow(xmlDoc);
	});
*/	$("#toolbar_btn_show_"+tempId).unbind("click").bind("click",function(){
		formatShow(xmlDoc);
	});
	/*$("#tools_"+tempId+" > div[type='SAVE']").unbind("click").bind("click",function(){
...
/*	$("#tools_"+tempId+" > div[type='VALIDATE']").unbind("click").bind("click",function(){
		var tab = $('#process_tabs').tabs("getSelected");
		validate(tab);
	});*/
	...
	function showDataFiled(){
		...
		$("#dataFieldBtn_import").unbind("click").bind("click",function(){
		    ...
		    $('#dataField_preview').datagrid({
		    	url: basePath + "/backstage/website/work-flow-define!importDataField.json?id=" + tabId,
		        columns:[[
...
var Rect = function(raphaelPaper,type,x,y,and,or,activity){
	...
	function showProps(){
		...
		var showNextPageEditor = performerControlEditor = {
				type : 'combobox',
				options:{
					data:[{
						"value":"是",
		...
		case "Participant":
			...
			if(typeValue == "HUMAN"){
				...
				if(exts != null){
					...
					for(var i=0;i<exts.childNodes.length;i++){
						var node = exts.childNodes[i];
						if(node.nodeName == "ExtendedAttribute" && node.getAttribute("name") == "CLFLOW_ROLE_ID"){
							departId = node.getAttribute("value");
						}
						if(node.nodeName == "ExtendedAttribute" && node.getAttribute("name") == "CLFLOW_ROLE_NAME"){
							departName = node.getAttribute("value");
						}
					}
					if(departId != null && departName != null){
						...
						if(exts != null){
							...
							if(type == "HUMAN"){
								...
								if(row != ""){
									...
									var name = rowArray[1];
									var ext = xmlDoc.createElement("ExtendedAttribute");
									ext.setAttribute("name","CLFLOW_ROLE_ID");
									ext.setAttribute("value",id);
									exts.appendChild(ext);
									ext = xmlDoc.createElement("ExtendedAttribute");
									ext.setAttribute("name","CLFLOW_ROLE_NAME");
									ext.setAttribute("value",name);
									exts.appendChild(ext);
								}
